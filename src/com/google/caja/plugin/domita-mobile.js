...
var attachDocumentStub = (function () {
    ...
    function attachDocumentStub(idSuffix, uriCallback, imports, pseudoBodyNode, optPseudoWindowLocation) {
        ...
        function wrapperPositionError(p){
            return ___.primFreeze({
                message: p.message,
                code: p.code,
                PERMISSION_DENIED: p.PERMISSION_DENIED,
                POSITION_UNAVAILABLE: p.POSITION_UNAVAILABLE,
                TIMEOUT: p.TIMEOUT
            });
        }
        ...
        var tameNavigator = ___.primFreeze({
            ...
            cajaVersion: '1.0',
            geolocation: {
                getCurrentPosition: ___.markFuncFreeze(function (success, error, options) {
                    window.navigator.geolocation.getCurrentPosition(function (p) {
                        success(wrapperGeoPosition(p));
                    }, function (e) {
                        error(wrapperPositionError(e));
                    }, options);
                }),
                watchPosition: ___.markFuncFreeze(function (success, error, options) {
                    return window.navigator.geolocation.watchPosition(function (p) {
                        success(wrapperGeoPosition(p));
                    }, function (e) {
                        error(wrapperPositionError(e));
                    }, options);
                }),
                clearWatch: ___.markFuncFreeze(function (id) {
